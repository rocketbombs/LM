...
class GraphReducer:
    ...
    def _reduce_at_path(self, graph: GraphNode, path: List[int]) -> GraphNode:
        ...
        if not path:
            ...
            # Create thunk with CURRENT environment, not graph.right.env
            # This ensures the argument is evaluated in the correct scope
            arg_thunk = GraphNode(NodeKind.THUNK, body=graph.right,
                                env=graph.env, evaluated=False)
...
def validate_mode(args, config: Config):
    ...
    print("\n[5] Renderer round-trip determinism")
    test_terms = []
    for _ in range(12):
    ...
    print(f"\n{'='*60}")
    if failed == 0:
        print(f"ALL TESTS PASSED ({passed}/6 test groups)")
        print('='*60)
        return True
    else:
